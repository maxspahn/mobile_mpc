...
a Python wrapper for a fast solver generated by FORCES PRO v3.1.0
   ...
   subject to the parameters supplied in the following dictionary:
       ...
       PARAMS['all_parameters'] - column vector of length 5200
...
try:
	_lib = ctypes.CDLL(os.path.join(os.path.dirname(os.path.abspath(__file__)),'mm_MPC/lib/mm_MPC_withModel.so'))
	csolver = getattr(_lib,'mm_MPC_solve')
except:
	_lib = ctypes.CDLL(os.path.join(os.path.dirname(os.path.abspath(__file__)),'mm_MPC/lib/libmm_MPC_withModel.so'))
	csolver = getattr(_lib,'mm_MPC_solve')
...
('all_parameters', ctypes.c_double * 5200),
]

mm_MPC_params = {'xinit' : np.array([]),
'x0' : np.array([]),
...
mm_MPC_params_types = {'xinit' : np.float64,
'x0' : np.float64,
...
('x16', ctypes.c_double * 20),
('x17', ctypes.c_double * 20),
('x18', ctypes.c_double * 20),
('x19', ctypes.c_double * 20),
('x20', ctypes.c_double * 20),
]

mm_MPC_outputs = {'x01' : np.array([]),
'x02' : np.array([]),
'x03' : np.array([]),
'x04' : np.array([]),
'x05' : np.array([]),
...
csolver.argtypes = ( ctypes.POINTER(mm_MPC_params_ctypes), ctypes.POINTER(mm_MPC_outputs_ctypes), ctypes.POINTER(mm_MPC_info), ctypes.POINTER(FILE))
 ...
 See also COPYING
	...
	for par in params_arg:
		try:
			...
			params_arg[par] = np.require(params_arg[par], dtype=mm_MPC_params_types[par], requirements='F')
	...
	if sys.version_info.major == 2:
		...
		try:
			PyFile_AsFile.restype = ctypes.POINTER(FILE)
			exitflag = _lib.mm_MPC_solve( params_py, ctypes.byref(outputs_py), ctypes.byref(info_py), PyFile_AsFile(fp) , _lib.mm_MPC_casadi2forces )
		...
		try:
			if sys.platform.startswith('win'):
				exitflag = _lib.mm_MPC_solve( params_py, ctypes.byref(outputs_py), ctypes.byref(info_py), None , _lib.mm_MPC_casadi2forces)
			else:
				exitflag = _lib.mm_MPC_solve( params_py, ctypes.byref(outputs_py), ctypes.byref(info_py), fp , _lib.mm_MPC_casadi2forces)
			libc.fclose(fp)
