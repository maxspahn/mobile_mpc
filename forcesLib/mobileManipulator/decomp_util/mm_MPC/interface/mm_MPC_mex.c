...
extern void mm_MPC_casadi2forces(mm_MPC_float *x, mm_MPC_float *y, mm_MPC_float *l, mm_MPC_float *p, mm_MPC_float *f, mm_MPC_float *nabla_f, mm_MPC_float *c, mm_MPC_float *nabla_c, mm_MPC_float *h, mm_MPC_float *nabla_h, mm_MPC_float *hess, solver_int32_default stage, solver_int32_default iteration);
mm_MPC_extfunc pt2function_mm_MPC = &mm_MPC_casadi2forces;


/* Some memory for mex-function */
static mm_MPC_params params;
static mm_MPC_output output;
static mm_MPC_info info;
...
void mexFunction( solver_int32_default nlhs, mxArray *plhs[], solver_int32_default nrhs, const mxArray *prhs[] )  
{
    ...
    if (nrhs != 1) 
	{
        mexErrMsgTxt("This function requires exactly 1 input: PARAMS struct.\nType 'help mm_MPC_mex' for details.");
    }    
	if (nlhs > 3) 
	{
    ...
    if( mxGetM(par) != 5200 || mxGetN(par) != 1 ) 
	{
    mexErrMsgTxt("PARAMS.all_parameters must be of size [5200 x 1]");
    }
#endif	 
	if ( (mxGetN(par) != 0) && (mxGetM(par) != 0) )
	{
		copyMArrayToC_double(mxGetPr(par), params.all_parameters,5200);

	}


	#if SET_PRINTLEVEL_mm_MPC > 0
	...
	/* call solver */
	exitflag = mm_MPC_solve(&params, &output, &info, fp, pt2function_mm_MPC);
	...
	copyCArrayToM_double( output.x01, mxGetPr(outvar), 20);
	mxSetField(plhs[0], 0, "x01", outvar);

	outvar = mxCreateDoubleMatrix(20, 1, mxREAL);
	copyCArrayToM_double( output.x02, mxGetPr(outvar), 20);
	mxSetField(plhs[0], 0, "x02", outvar);

	outvar = mxCreateDoubleMatrix(20, 1, mxREAL);
	copyCArrayToM_double( output.x03, mxGetPr(outvar), 20);
	mxSetField(plhs[0], 0, "x03", outvar);

	outvar = mxCreateDoubleMatrix(20, 1, mxREAL);
	copyCArrayToM_double( output.x04, mxGetPr(outvar), 20);
	mxSetField(plhs[0], 0, "x04", outvar);

	outvar = mxCreateDoubleMatrix(20, 1, mxREAL);
	copyCArrayToM_double( output.x05, mxGetPr(outvar), 20);
	mxSetField(plhs[0], 0, "x05", outvar);

	outvar = mxCreateDoubleMatrix(20, 1, mxREAL);
	copyCArrayToM_double( output.x06, mxGetPr(outvar), 20);
	mxSetField(plhs[0], 0, "x06", outvar);

	outvar = mxCreateDoubleMatrix(20, 1, mxREAL);
	copyCArrayToM_double( output.x07, mxGetPr(outvar), 20);
	mxSetField(plhs[0], 0, "x07", outvar);

	outvar = mxCreateDoubleMatrix(20, 1, mxREAL);
	copyCArrayToM_double( output.x08, mxGetPr(outvar), 20);
	mxSetField(plhs[0], 0, "x08", outvar);

	outvar = mxCreateDoubleMatrix(20, 1, mxREAL);
	copyCArrayToM_double( output.x09, mxGetPr(outvar), 20);
	mxSetField(plhs[0], 0, "x09", outvar);

	outvar = mxCreateDoubleMatrix(20, 1, mxREAL);
	copyCArrayToM_double( output.x10, mxGetPr(outvar), 20);
	mxSetField(plhs[0], 0, "x10", outvar);

	outvar = mxCreateDoubleMatrix(20, 1, mxREAL);
	copyCArrayToM_double( output.x11, mxGetPr(outvar), 20);
	mxSetField(plhs[0], 0, "x11", outvar);

	outvar = mxCreateDoubleMatrix(20, 1, mxREAL);
	copyCArrayToM_double( output.x12, mxGetPr(outvar), 20);
	mxSetField(plhs[0], 0, "x12", outvar);

	outvar = mxCreateDoubleMatrix(20, 1, mxREAL);
	copyCArrayToM_double( output.x13, mxGetPr(outvar), 20);
	mxSetField(plhs[0], 0, "x13", outvar);

	outvar = mxCreateDoubleMatrix(20, 1, mxREAL);
	copyCArrayToM_double( output.x14, mxGetPr(outvar), 20);
	mxSetField(plhs[0], 0, "x14", outvar);

	outvar = mxCreateDoubleMatrix(20, 1, mxREAL);
	copyCArrayToM_double( output.x15, mxGetPr(outvar), 20);
	mxSetField(plhs[0], 0, "x15", outvar);

	outvar = mxCreateDoubleMatrix(20, 1, mxREAL);
	copyCArrayToM_double( output.x16, mxGetPr(outvar), 20);
	mxSetField(plhs[0], 0, "x16", outvar);

	outvar = mxCreateDoubleMatrix(20, 1, mxREAL);
	copyCArrayToM_double( output.x17, mxGetPr(outvar), 20);
	mxSetField(plhs[0], 0, "x17", outvar);

	outvar = mxCreateDoubleMatrix(20, 1, mxREAL);
	copyCArrayToM_double( output.x18, mxGetPr(outvar), 20);
	mxSetField(plhs[0], 0, "x18", outvar);

	outvar = mxCreateDoubleMatrix(20, 1, mxREAL);
	copyCArrayToM_double( output.x19, mxGetPr(outvar), 20);
	mxSetField(plhs[0], 0, "x19", outvar);

	outvar = mxCreateDoubleMatrix(20, 1, mxREAL);
	copyCArrayToM_double( output.x20, mxGetPr(outvar), 20);
