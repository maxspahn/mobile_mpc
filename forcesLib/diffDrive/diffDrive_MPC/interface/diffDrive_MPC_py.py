...
a Python wrapper for a fast solver generated by FORCES PRO v3.0.1
   ...
   OUTPUT = diffDrive_MPC_py.diffDrive_MPC_solve(PARAMS) solves a multistage problem
   subject to the parameters supplied in the following dictionary:
       PARAMS['xinit'] - column vector of length 5
       PARAMS['x0'] - column vector of length 75
       PARAMS['all_parameters'] - column vector of length 150
   ...
   [OUTPUT, EXITFLAG] = diffDrive_MPC_py.diffDrive_MPC_solve(PARAMS) returns additionally
   ...
   [OUTPUT, EXITFLAG, INFO] = diffDrive_MPC_py.diffDrive_MPC_solve(PARAMS) returns 
...
#_lib = ctypes.CDLL(os.path.join(os.getcwd(),'diffDrive_MPC/lib/diffDrive_MPC.so')) 
try:
	_lib = ctypes.CDLL(os.path.join(os.path.dirname(os.path.abspath(__file__)),'diffDrive_MPC/lib/diffDrive_MPC_withModel.so'))
	csolver = getattr(_lib,'diffDrive_MPC_solve')
except:
	_lib = ctypes.CDLL(os.path.join(os.path.dirname(os.path.abspath(__file__)),'diffDrive_MPC/lib/libdiffDrive_MPC_withModel.so'))
	csolver = getattr(_lib,'diffDrive_MPC_solve')

class diffDrive_MPC_params_ctypes(ctypes.Structure):
#	@classmethod
#	def from_param(self):
#		return self
	_fields_ = [('xinit', ctypes.c_double * 5),
('x0', ctypes.c_double * 75),
('all_parameters', ctypes.c_double * 150),
]

diffDrive_MPC_params = {'xinit' : np.array([]),
'x0' : np.array([]),
...
diffDrive_MPC_params_types = {'xinit' : np.float64,
'x0' : np.float64,
'all_parameters' : np.float64,
}

class diffDrive_MPC_outputs_ctypes(ctypes.Structure):
#	@classmethod
#	def from_param(self):
#		return self
...
diffDrive_MPC_outputs = {'x01' : np.array([]),
'x02' : np.array([]),
'x03' : np.array([]),
'x04' : np.array([]),
'x05' : np.array([]),
...
class diffDrive_MPC_info(ctypes.Structure):
#	@classmethod
#	def from_param(self):
#		return self
...
csolver.argtypes = ( ctypes.POINTER(diffDrive_MPC_params_ctypes), ctypes.POINTER(diffDrive_MPC_outputs_ctypes), ctypes.POINTER(diffDrive_MPC_info), ctypes.POINTER(FILE))
csolver.restype = ctypes.c_int

def diffDrive_MPC_solve(params_arg):
	'''
a Python wrapper for a fast solver generated by FORCES PRO v3.0.1
 ...
 See also COPYING
	...
	# convert parameters
	params_py = diffDrive_MPC_params_ctypes()
	for par in params_arg:
		try:
			#setattr(params_py, par, npct.as_ctypes(np.reshape(params_arg[par],np.size(params_arg[par]),order='A'))) 
			params_arg[par] = np.require(params_arg[par], dtype=diffDrive_MPC_params_types[par], requirements='F')
	...
	outputs_py = diffDrive_MPC_outputs_ctypes()
	info_py = diffDrive_MPC_info()
	if sys.version_info.major == 2:
		...
		try:
			PyFile_AsFile.restype = ctypes.POINTER(FILE)
			exitflag = _lib.diffDrive_MPC_solve( params_py, ctypes.byref(outputs_py), ctypes.byref(info_py), PyFile_AsFile(fp) , _lib.diffDrive_MPC_casadi2forces )
		...
		try:
			if sys.platform.startswith('win'):
				exitflag = _lib.diffDrive_MPC_solve( params_py, ctypes.byref(outputs_py), ctypes.byref(info_py), None , _lib.diffDrive_MPC_casadi2forces)
			else:
				exitflag = _lib.diffDrive_MPC_solve( params_py, ctypes.byref(outputs_py), ctypes.byref(info_py), fp , _lib.diffDrive_MPC_casadi2forces)
			libc.fclose(fp)
	...
	# convert outputs
	for out in diffDrive_MPC_outputs:
		diffDrive_MPC_outputs[out] = npct.as_array(getattr(outputs_py,out))

	return diffDrive_MPC_outputs,int(exitflag),info_py

solve = diffDrive_MPC_solve
