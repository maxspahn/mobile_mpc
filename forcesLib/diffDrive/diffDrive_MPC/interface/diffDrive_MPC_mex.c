...
extern void diffDrive_MPC_casadi2forces(diffDrive_MPC_float *x, diffDrive_MPC_float *y, diffDrive_MPC_float *l, diffDrive_MPC_float *p, diffDrive_MPC_float *f, diffDrive_MPC_float *nabla_f, diffDrive_MPC_float *c, diffDrive_MPC_float *nabla_c, diffDrive_MPC_float *h, diffDrive_MPC_float *nabla_h, diffDrive_MPC_float *hess, solver_int32_default stage, solver_int32_default iteration);
...
void mexFunction( solver_int32_default nlhs, mxArray *plhs[], solver_int32_default nrhs, const mxArray *prhs[] )  
{
    ...
    if (nrhs != 1) 
	{
        mexErrMsgTxt("This function requires exactly 1 input: PARAMS struct.\nType 'help diffDrive_MPC_mex' for details.");
    }    
	if (nlhs > 3) 
	{
    ...
    if( mxGetM(par) != 75 || mxGetN(par) != 1 ) 
	{
    mexErrMsgTxt("PARAMS.x0 must be of size [75 x 1]");
    }
#endif	 
	if ( (mxGetN(par) != 0) && (mxGetM(par) != 0) )
	{
		copyMArrayToC_double(mxGetPr(par), params.x0,75);
