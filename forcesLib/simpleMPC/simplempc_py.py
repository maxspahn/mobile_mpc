...
a Python wrapper for a fast solver generated by FORCES PRO v4.0.0
   ...
   OUTPUT = simplempc_py.simplempc_solve(PARAMS) solves a multistage problem
   ...
   [OUTPUT, EXITFLAG] = simplempc_py.simplempc_solve(PARAMS) returns additionally
   ...
   [OUTPUT, EXITFLAG, INFO] = simplempc_py.simplempc_solve(PARAMS) returns 
...
#_lib = ctypes.CDLL(os.path.join(os.getcwd(),'simplempc/lib/simplempc.so')) 
try:
	_lib = ctypes.CDLL(os.path.join(os.path.dirname(os.path.abspath(__file__)),'simplempc/lib/simplempc_withModel.so'))
	csolver = getattr(_lib,'simplempc_solve')
except:
	_lib = ctypes.CDLL(os.path.join(os.path.dirname(os.path.abspath(__file__)),'simplempc/lib/libsimplempc_withModel.so'))
	csolver = getattr(_lib,'simplempc_solve')

class simplempc_params_ctypes(ctypes.Structure):
#	@classmethod
#	def from_param(self):
#		return self
	_fields_ = [('xinit', ctypes.c_double * 20),
('x0', ctypes.c_double * 300),
('all_parameters', ctypes.c_double * 5085),
]

simplempc_params = {'xinit' : np.array([]),
'x0' : np.array([]),
...
simplempc_params_types = {'xinit' : np.float64,
'x0' : np.float64,
'all_parameters' : np.float64,
}

class simplempc_outputs_ctypes(ctypes.Structure):
#	@classmethod
#	def from_param(self):
#		return self
	_fields_ = [('x01', ctypes.c_double * 20),
('x02', ctypes.c_double * 20),
('x03', ctypes.c_double * 20),
('x04', ctypes.c_double * 20),
('x05', ctypes.c_double * 20),
('x06', ctypes.c_double * 20),
('x07', ctypes.c_double * 20),
('x08', ctypes.c_double * 20),
('x09', ctypes.c_double * 20),
('x10', ctypes.c_double * 20),
('x11', ctypes.c_double * 20),
('x12', ctypes.c_double * 20),
('x13', ctypes.c_double * 20),
('x14', ctypes.c_double * 20),
('x15', ctypes.c_double * 20),
]

simplempc_outputs = {'x01' : np.array([]),
'x02' : np.array([]),
'x03' : np.array([]),
'x04' : np.array([]),
'x05' : np.array([]),
...
class simplempc_info(ctypes.Structure):
#	@classmethod
#	def from_param(self):
#		return self
...
csolver.argtypes = ( ctypes.POINTER(simplempc_params_ctypes), ctypes.POINTER(simplempc_outputs_ctypes), ctypes.POINTER(simplempc_info), ctypes.POINTER(FILE))
csolver.restype = ctypes.c_int

def simplempc_solve(params_arg):
	'''
a Python wrapper for a fast solver generated by FORCES PRO v4.0.0
 ...
 See also COPYING
	...
	# convert parameters
	params_py = simplempc_params_ctypes()
	for par in params_arg:
		try:
			#setattr(params_py, par, npct.as_ctypes(np.reshape(params_arg[par],np.size(params_arg[par]),order='A'))) 
			params_arg[par] = np.require(params_arg[par], dtype=simplempc_params_types[par], requirements='F')
	...
	outputs_py = simplempc_outputs_ctypes()
	info_py = simplempc_info()
	if sys.version_info.major == 2:
		...
		try:
			PyFile_AsFile.restype = ctypes.POINTER(FILE)
			exitflag = _lib.simplempc_solve( params_py, ctypes.byref(outputs_py), ctypes.byref(info_py), PyFile_AsFile(fp) , _lib.simplempc_casadi2forces )
		...
		try:
			if sys.platform.startswith('win'):
				exitflag = _lib.simplempc_solve( params_py, ctypes.byref(outputs_py), ctypes.byref(info_py), None , _lib.simplempc_casadi2forces)
			else:
				exitflag = _lib.simplempc_solve( params_py, ctypes.byref(outputs_py), ctypes.byref(info_py), fp , _lib.simplempc_casadi2forces)
			libc.fclose(fp)
	...
	# convert outputs
	for out in simplempc_outputs:
		simplempc_outputs[out] = npct.as_array(getattr(outputs_py,out))

	return simplempc_outputs,int(exitflag),info_py

solve = simplempc_solve
